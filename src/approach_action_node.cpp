...
class ApproachAction : public plansys2::ActionExecutorClient
{
public:
  ApproachAction()
  : plansys2::ActionExecutorClient("approach")
  {
    using namespace std::placeholders;
  ...
  on_activate(const rclcpp_lifecycle::State & previous_state)
  {
    ...
    pos_sub_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
      "/amcl_pose",
      10,
      std::bind(&ApproachAction::current_pos_callback, this, _1));
    
    params_client_ = shared_from_this()->create_client<GetParameters>(
      "/global_costmap/global_costmap/get_parameters");
    private_node_ = rclcpp::Node::make_shared("approach_action_pub");
    action_pub_ =
...
private:
  ...
  void getNavApproachPosesFromAgent(
    std::string id,
  ...
  void actionStep()
  {
    if (!action_setted && get_current_state().label() == "active") {
      ...
      message.action = "approaching";
      action_pub_->publish(message);
      action_setted = true;
    }

    poses_.clear();
    getNavApproachPosesFromAgent(agent_id_, poses_);
    geometry_msgs::msg::Pose approach_p;
    if (poses_.size() == 0 || !getBetterPose(poses_, approach_p)) {
      return;
    }

    if (dist_to_move == 0.0) {
      dist_to_move = getDistance(approach_p, current_pos_);
    }
    ...
    //RCLCPP_INFO(get_logger(), "progress [%f]", getFeedback()->progress);
  ...
  bool isFinished()
  {
     if (getFeedback()->progress == 100.0) {
      !action_setted;
      navigation_goal_.pose.pose = geometry_msgs::msg::Pose();
  ...
  bool action_setted, goal_sended;
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<ApproachAction>();
